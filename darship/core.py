...
class PartnershipBot(commands.AutoShardedInteractionBot):
    def __init__(self, *, mongo_uri: str, loop: asyncio.AbstractEventLoop) -> None:
        ...
        self.database = DatabaseManager(AsyncIOMotorClient(mongo_uri, io_loop=self.loop))
    ...
    def tr(self, locale: str, key: str) -> str:
        ...
        try:
            ...
            if ls:
                return ls.get(locale or 'en-US') or ls.get('en-US')
            return key
        except:
            return key
    ...
    async def begin_interaction(self, inter: disnake.Interaction, *, update: dict[str, Any] = {}, raw_update: dict[str, Any] = {}) -> tuple[Optional[Guild], Callable[[str], str]]:
        if inter.guild_id:
            guild = await self.database.guild(inter.guild_id, update={
                '$set': update,
            } | raw_update)
            return guild, self.make_tr(guild.language)
